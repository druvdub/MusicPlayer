...
class MainView(QMainWindow, Ui_MainWindow):
    def __init__(self, *args, **kwargs):
        ...
        self.playlist.currentIndexChanged.connect(self.selectionChanged)
    ...
    def removeFromQueue(self):
        for indec in sorted(self.currentPlayList.selectionModel().selectedRows()):
            i = index.row()
            if i > -1:
    ...
    def saveCurrentPlaylist(self):
        ...
        if ok: 
            songs = []
            for idx in range(self.playlist.mediaCount()):
                path = self.playlist.media(idx).canonicalUrl().path()
                id = db.get_song_by_address(path)[0][0]
                songs.append(id)
            db.add_playlist(text, songs)
        self.playlistListModel.updateData()
    ...
    def addFileToDatabase(self, address):
        if len(db.get_song_by_address(address)) == 0:
            ...
            db.insert_song(metadata.title, metadata.artist, metadata.year, metadata.album, address, metadata.genre)
            self.songListModel.updData()

    def removeSongsFromDatabase(self):
        for idx in sorted(self.songs.selectionModel().selectedRows()):
            idx = self.songListProxyModel.mapToSource(index)
            i = idx.row()
            db.delete_song_by_id(self.songListModel.getId(idx))
        self.songListModel.updData()
    ...
    def addSongstoPlaylist(self):
        for index in sorted(self.songs.selectionModel().selectedRows()):
            ...
            self.playlist.addMedia(QMediaContent(QUrl.fromLocalFile(self.songListModel.data(self.songListModel.index(index.row(), 4)))))
        self.playlistModel.layoutChanged.emit()
    ...
    def removePlaylistfromDatabase(self):
        for idx in sorted(self.playlistList.selectionModel().selectedRows()):
            db.delete_playlist(self.playlistListModel.getid(index))
        self.playlistListModel.updateData()

    def addPlaylistToQueue(self):
        for idx in sorted(self.playlistList.selectionModel().selectedRows()):
            dt = db.get_playlist(self.playlistListModel.getid(idx))
            for line in dt:
    ...
    def dropEvent(self, e):
        for url in e.mimeData().urls():
            self.addFileToDatabase(url.path())
